...
describe("persist-inputs", () => {
  it("mergeSavedInputs ignores unknown keys and invalid enums", () => {
    const merged = mergeSavedInputs({
      global: { province: "ZZ", annualKm: 15_000 },
      extra: 123,
    });
  ...
  it("migrates legacy global target to per-scenario cash target", () => {
    const merged = mergeSavedInputs({
      global: { province: "BC", targetAllInMonthly: 900, ownershipHorizonMonths: 48 },
      scenarios: [
        {
          id: "x",
          name: "Test",
    ...
    assert.equal(merged.global.targetEconomicMonthly, undefined);
    assert.equal(merged.scenarios[0].targetCashAllInMonthly, 900);
    assert.equal(merged.scenarios[0].ownershipHorizonMonths, 48);
  ...
  it("sanitizes retained value and horizon bounds", () => {
    const merged = mergeSavedInputs({
      scenarios: [
        createScenario({
          retainedValuePercent: { mode: "manual", manual: -10 },
          vehicleAgeYears: 3,
          ownershipHorizonMonths: 999,
          targetCashAllInMonthly: 750,
        }),
      ],
      current: { retainedValuePercent: 150, marketValue: 22_000, vehicleAgeYears: 7 },
    });
    assert.equal(merged.scenarios[0].ownershipHorizonMonths, 120);
    assert.equal(merged.scenarios[0].targetCashAllInMonthly, 750);
  ...
  it("round-trips through serialize and parse", () => {
    ...
    custom.scenarios[0].targetCashAllInMonthly = 750;
    ...
    assert.equal(restored.scenarios[0].targetCashAllInMonthly, 750);
